...
namespace Perf.Util
{
     public class SimpleIntPairDictionary<TValue>:IDisposable
    {
    public SimpleIntPairDictionary() : this(11)
    {
    }

    public SimpleIntPairDictionary(int capacity)
    {
        _Reverse(capacity);
    }
    ...
    public TValue this[(int,int) key]
    {
        get
        {
            TValue value;
    ...
    public void Add((int,int) key, TValue value)
    {
    ...
    public bool Remove((int,int) key)
    {
        int hash = key.GetHashCode() & 0x7FFFFFFF;
        return _Remove(hash, key);
    }

    public bool TryGetValue((int,int) key, out TValue value)
    {
        int hash = key.GetHashCode() & 0x7FFFFFFF;
        int slot = _FindSlot(hash, key);
        if (slot != -1)
    ...
    public bool TryAdd((int,int) key, TValue value)
    {
        int hash = key.GetHashCode() & 0x7FFFFFFF;
        int slot = _FindSlot(hash, key);
        if (slot == -1)
    ...
    public TValue GetValueOrDefault((int,int) key, TValue defaultValue = default(TValue))
    {
        int hash = key.GetHashCode() & 0x7FFFFFFF;
        int slot = _FindSlot(hash, key);
        if (slot != -1)
    ...
    public SimpleIntPairDictionary<TValue>.Enumerator GetEnumerator()
    {
        return new Enumerator(this);
    }

    [Serializable]
    public struct Enumerator : IEnumerator, IDisposable, IEnumerator<KeyValuePair<(int,int), TValue>>
    {
        public KeyValuePair<(int,int), TValue> Current
        {
            get
            {
                return m_current;
        ...
        {
            m_dict = null;
            m_current = new KeyValuePair<(int,int), TValue>();
            m_index = 0;
        }
        ...
        public bool MoveNext()
        {
            ...
            {
                m_current = new KeyValuePair<(int,int), TValue>(m_dict.m_entryKey[m_index], m_dict.m_entryValue[m_index]);
                m_index++;
                return true;
            }
        ...
        {
            get { return new KeyValuePair<(int,int), TValue>(m_current.Key, m_current.Value); }
        }
        ...
        internal Enumerator(SimpleIntPairDictionary<TValue> dict)
        {
            m_dict = dict;
            m_current = new KeyValuePair<(int,int), TValue>();
            m_index = 0;
        ...
        private int m_index;
        private SimpleIntPairDictionary<TValue> m_dict;
        private KeyValuePair<(int,int), TValue> m_current;
    }
    public (int,int)[] Keys
    {
        get
        {
            var keys = new (int,int)[m_count];
    ...
    private void _Reverse(int size)
    {
        ...
        (int,int)[] tempEntrykey = m_entryKey;
    ...
    private void _AddNoCheck(int hash, (int,int) key, TValue value, int pos)
    {
    ...
    private int _FindSlot(int hash, (int,int) key)
    {
        int slot = hash % m_mod;
    ...
    private bool _Remove(int hash, (int,int) key)
    {
        int slot = hash % m_mod;
        ...
        m_entryKey[m_count] = default((int,int));
    ...
    private (int,int)[] m_entryKey = null;
