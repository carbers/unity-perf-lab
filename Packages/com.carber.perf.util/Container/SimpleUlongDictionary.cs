...
namespace Perf.Util
{
    public class SimpleUlongDictionary<TValue>:IDisposable
    {
    public SimpleUlongDictionary() : this(11)
    {
    }

    public SimpleUlongDictionary(int capacity)
    {
        _Reverse(capacity);
    }
    ...
    public TValue GetValueOrDefault(ulong key, TValue defaultValue = default(TValue))
    {
        int hash = key.GetHashCode() & 0x7FFFFFFF;
        int slot = _FindSlot(hash, key);
        if (slot != -1)
    ...
    public SimpleUlongDictionary<TValue>.Enumerator GetEnumerator()
    {
        return new Enumerator(this);
    }

    [Serializable]
    public struct Enumerator : IEnumerator, IDisposable, IEnumerator<KeyValuePair<ulong, TValue>>
    {
        public KeyValuePair<ulong, TValue> Current
        {
            get
            {
                return m_current;
        ...
        {
            m_dict = null;
            m_current = new KeyValuePair<ulong, TValue>();
            m_index = 0;
        }
        ...
        public bool MoveNext()
        {
            ...
            {
                m_current = new KeyValuePair<ulong, TValue>(m_dict.m_entryKey[m_index], m_dict.m_entryValue[m_index]);
                m_index++;
                return true;
            }
        ...
        {
            get { return new KeyValuePair<ulong, TValue>(m_current.Key, m_current.Value); }
        }
        ...
        internal Enumerator(SimpleUlongDictionary<TValue> dict)
        {
            m_dict = dict;
            m_current = new KeyValuePair<ulong, TValue>();
            m_index = 0;
        ...
        private int m_index;
        private SimpleUlongDictionary<TValue> m_dict;
        private KeyValuePair<ulong, TValue> m_current;
    }
    public ulong[] Keys
    {
        get
        {
            var keys = new ulong[m_count];
    ...
    private void _AddNoCheck(int hash, ulong key, TValue value, int pos)
    {
    ...
    private int _FindSlot(int hash, ulong key)
    {
        int slot = hash % m_mod;
    ...
    private bool _Remove(int hash, ulong key)
    {
        int slot = hash % m_mod;
        ...
        m_entryKey[m_count] = default(ulong);
